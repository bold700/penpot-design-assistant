...
<!-- Scraped: 2026-04-20T08:18:10.830Z -->
...
Copy link
Link copied
When actions don’t fit in a toolbar, add a 
...
The toolbar and 
   ...
   are both placed at the bottom of the window, so should not be shown at the same time. Show the navigation bar on primary pages, and toolbars on subsequent pages with actions.
link
Copy link
Link copied
...
When configuring a toolbar, think of it as a container with several slots.These slots can be populated by 
...
- Use different 
   ...
   color styles, such as filled, tonal, and standard
- Customize the 
   ...
   of a single action, such as a primary or secondary palette
...
A 
   ...
   can be placed next to a floating toolbar to present one high-priority action alongside a unified set of toolbar actions.
...
Copy link
Link copied
Vertical toolbars should be positioned opposite the 
   ...
   to balance out the screen and keep actions easy to access.
...
In 
  ...
  , elements in the toolbar should be evenly spaced.
In 
   ...
   and larger, adjust the padding between controls to create a comfortable layout. This can be achieved by:
