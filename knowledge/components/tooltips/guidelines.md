...
Plain tooltipsPlain tooltips briefly describe a UI element. They're best used for labelling UI elements with no text, like icon-only 
   ...
   and fields.
Rich tooltipsRich tooltips provide additional context about a UI element. They can optionally contain a subhead, buttons, and hyperlinks.
...
Rich tooltips can have up to two text 
  ...
  . These should be brief and relevant to the message in the supporting text.
...
To show a tooltip, 
   ...
   on the parent element on desktop, or tap and hold the element on mobile. Persistent 
